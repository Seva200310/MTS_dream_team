...
class Robot_controller():
    ...
    def get_sensor_data_converted(self):#функция которая возвращает sensor_data относительно ячееки матрицы
        ...
        data_dist = [data["front_distance"],data["right_distance"],data["back_distance"],data["left_distance"]]
        converted_data_dist = []
    ...
    def get_current_wall_config(self):#функция возвращающая положение стен в текущей конфигурации
        #Получаем преобразованные данные с датчиков
        data = self.get_sensor_data_converted()
        data_dist = [data["front_distance"],data["right_distance"],data["back_distance"],data["left_distance"]]
        wall_config = []
        for distance in data_dist:
            if distance == 0:#там где расстояние равно 5 стена
